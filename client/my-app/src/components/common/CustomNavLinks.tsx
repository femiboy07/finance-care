...
const CustomNavLinks:React.FC<CustomNavProps>=({to,children,exact=false,title})=>{
    ...
    return(
        <NavLink to={to}  title={title}  className={`min-h-[40px]  px-3  py-2  cursor-pointer flex items-center w-full justify-start  ${isActive ? 'bg-orange-300   ':" "} `} >
          {React.Children.map(children,cloneWithProps)}
        </NavLink>
        // <NavLink to={to}  title={title}   className={`min-h-[40px]  px-3  py-2  rounded-md cursor-pointer flex items-center w-full justify-start ${isActive ? 'bg-orange-300 text-orange-600  ':" "} `}
        //   >
        //  {React.Children.map(children,cloneWithProps)}
